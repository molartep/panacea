...
lims_C1J <- as.POSIXct(strptime(c("2018-09-15 01:00","2019-03-20 01:00"), format = "%Y-%m-%d %H:%M"))
...
lims_L2E <- as.POSIXct(strptime(c("2018-08-15 01:00","2019-02-20 01:00"), format = "%Y-%m-%d %H:%M"))
...
H1A_df_stats <- data.frame(Interval = c("1st", "2nd", "3rd", "4th", "5th"),
                           ...
                           Pval = formatC(c(signif(H1_A_pvalues[c(1,2)], 4), NA, signif(H1_A_estims[c(3,4)], 4)),  format = "e", digits = 4),
...
grid.arrange(first_graph_C1_J, second_graph_C1_J, third_graph_C1_J, fourth_graph_C1_J, fifth_graph_C1_J, ncol=2, left = "Polarity", bottom = "Total Treatment Duration (Hrs)")
```
...
<ul>
  ...
  <li>Light blue shades represent treatment sessions.</li>
</ul>
</font>

<br />

```{r C1J_signif, out.width="100%", fig.height=5.2}
library(timevis)
library(dplyr)
...
C1J_signif <- data.frame(
  id = 1:41,
  content = c("", "", "", "", "",
              "0.5 cm", "0.8 cm", "SUV max: 2.37", "0.7 cm", "0.5 cm", "SUV max: 2.3", 
              "SUV max: 2.96", "2.7 cm", "1.4 x 1.1 cm", "SUV max: 5.2",
              "SUV max: 1.98", "SUV max: 1.39",
              "4.3 cm", "4.1 x 3.4 cm", "4.2 x 3.2 cm", "3.6 x 3.0 cm",
              "2.3 x 2.2 cm", "SUV max: 7.2", "5.9 x 5.0 cm", "SUV max: 6.9",
              "0.7 cm", "SUV max: 4.07", "1.1 cm", "1.0 x 0.6 cm", "SUV max: 2.7",
              "2.2 x 2.6 cm", "SUV max: 9.22", "3.2 x 3.2 cm", "SUV max: 10.2", "4.3 x 4.1 cm", "SUV max: 8.4",
              "3.3 x 2.3 cm", "SUV max: 7.43", "3.9 x 2.6 cm", "4.0 x 3.5 cm", "SUV max: 6.2"),
  
  start = c("2018-09-24", "2018-10-22", "2018-11-13", "2018-12-03", "2019-02-04",
            "2018-03-01", rep("2018-07-12", 2), "2018-10-19", rep("2019-01-28", 2),
            "2018-07-12", "2018-10-19", rep("2019-01-28", 2),
            "2018-07-12", "2019-01-28",
            "2018-03-01", "2018-07-12", "2018-10-19", "2019-01-28",
            rep("2018-10-19", 2), rep("2019-01-28", 2),
            rep("2018-07-12", 2), "2018-10-19", rep("2019-01-28", 2),
            rep("2018-07-12", 2), rep("2018-10-19", 2), rep("2019-01-28", 2),
            rep("2018-07-12", 2), "2018-10-19", rep("2019-01-28", 2)),
  ...
  group = c(rep(NA, 5), rep("rul", 6), rep("lhilum", 4), rep("liver", 2), rep("hs67", 4), rep("hs8", 4), rep("para", 5), rep("soft", 6), rep("softPiri", 5)),
  ...
  type = c(rep("background", 5), rep("point", 36))
)

C1J_groups1 <- data.frame(
  id = c("rul", "lhilum", "liver", "hs67", "hs8", "para", "soft", "softPiri"),
  content = c("Lung Right Upper Lobe Nodule", "Left Hilar\n Lymph Node", "Liver Lesion", "Hepatic Segment 6/7 Lesion", "Hepatic Segment 8 Lesion", "Para-aortic Lymph Node", "Soft Tissue Lymph Node", "Soft Tissue Lymph Node\n (Abutting the Piriformis)"),
  subgroupStack = TRUE
)
...
<br />
* The <b>Soft Tissue Lymph Node (Abutting the Piriformis)</b> is located to the left iliacus muscle and abutting the piriformis.
<br />
...
Start Date    | End Date      | Treatment
:-----------: | :-----------: | :-----------:
Content Cell  | Content Cell  | Left-Breast Chemotherapy
Content Cell  | Content Cell  | Right-Breast Chemotherapy
Content Cell  | Content Cell  | Left Masectomy
...
grid.arrange(top = "L2-E Polarity", first_graph_L2E, second_graph_L2E, third_graph_L2E, fourth_graph_L2E, fifth_graph_L2E, ncol=2, left = "Polarity", bottom = "Total Treatment Duration (Hrs)")
```
...
grid.arrange(top = "H1-A Polarity", first_graph_H1A, second_graph_H1A, third_graph_H1A, fourth_graph_H1A, fifth_graph_H1A, ncol=2, left = "Polarity", bottom = "Total Treatment Duration (Hrs)")
```
...
grid.arrange(top = "L3-K Polarity", first_graph_L3_K, second_graph_L3_K, third_graph_L3_K, fourth_graph_L3_K, ncol=2, left = "Polarity", bottom = "Total Treatment Duration (Hrs)")
```
