...
lims <- as.POSIXct(strptime(c("2018-10-25 01:00","2019-03-20 01:00"), format = "%Y-%m-%d %H:%M"))
...
L3K_first_day <- as.POSIXct(first_L3_K$Day[which(first_L3_K$`Total therapy duration (Hrs)`>0)][1],
                            origin = '1970-01-01')
L3K_last_day <- as.POSIXct(tail(fourth_L3_K$Day[which(fourth_L3_K$`Total therapy duration (Hrs)`>0)],1),
                           origin = '1970-01-01')
...
L3K_final_date_graph <- L3K_dategraph + geom_text(aes(x = L3K_first_day, y = first_L3_K$`Polarity level`[which(first_L3_K$`Polarity level`>0)][1],
  ...
  geom_text(aes(x = L3K_last_day, y = tail(fourth_L3_K$`Polarity level`[which(fourth_L3_K$`Polarity level`>0)],1),
  ...
  geom_text(aes(x = L3K_lab3x, y = L3K_lab3y + 5, color = "23", label = "+ 10.6",
                angle = 8)) + labs(color = "Days Without\nTreatment")
