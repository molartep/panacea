...
L3K_segment1 <- list(x1 = L3K_dates$date[7], y1 = L3K_dates$p_level[7],
                     x2 = L3K_dates$date[], y2 = L3K_dates$p_level[]) 
...
L3K_dategraph + geom_segment(aes(x = L3K_segment1$x1, y = L3K_segment1$y1,
                                 xend = L3K_segment1$x2, yend = L3K_segment1$y2, color = ""),
                             linetype = "dashed") +
  geom_text(aes(x = L3K_lab1x, y = L3K_lab1y + 5, color = "", label = "+ ",
                angle = )) + labs(color = "Days Without\nTreatment")